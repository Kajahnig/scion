...
var (
	remote snet.Addr
	//todo ev. add flags/vars for configDir and the file name
)
...
func realMain() int {
	...
	defer log.Flush()
	addFlags()
	integration.SetupWithFilters(createFilters())
	validateFlags()
...
func createFilters() []*filters.PacketFilter {
	var packetFilter filters.PacketFilter
	filter, err := path_length.NewPathLengthFilterFromStrings([]string{"-max", "5"})
	if err != nil {
		integration.LogFatal("Unable to create path length filter")
	}
	packetFilter = filters.PacketFilter(filter)
	return []*filters.PacketFilter{&packetFilter}
}
...
func (c client) attemptRequest(n int) bool {
	...
	// Receive pong TODO: adapt this to be okay with SCPM in certain cases
